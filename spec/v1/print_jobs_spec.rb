...
RSpec.describe V1::PrintJobsController, type: :request, helpers: true do
  ...
  it "should print a valid label" do
    ...
    post v1_print_jobs_path, params: {data: {attributes: { printer_name: printer.name, label_template_id: label_template.id, labels: label_template.dummy_labels.to_h}}}.to_json, headers: json_spec_headers
  ...
  it "should return an error if the label is not valid" do
    post v1_print_jobs_path, params: {data: {attributes: { label_template_id: label_template.id, labels: label_template.dummy_labels.to_h}}}.to_json, headers: json_spec_headers
  ...
  it "should return an error if request provides incorrect parameters" do
    post v1_print_jobs_path, params: {data: {attributes: { printer_name: printer.name, label_type_id: label_template.id, labels: label_template.dummy_labels.to_h}}}.to_json, headers: json_spec_headers
    expect(response).to have_http_status(:unprocessable_entity)
